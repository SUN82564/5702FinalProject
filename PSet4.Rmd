...
```
  \
Fortunately, the data which the website provided is in xlsx and provided with detailed explanation about the variables inside the data. It could be easily converted to csv file which means that reading by Rstudio is straight forward. After finishing loading the data, we found that the data is already tidy. By now, we only need to change missing value into *NA* and remove those *NA* values. \
